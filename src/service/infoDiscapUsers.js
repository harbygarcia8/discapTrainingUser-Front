...
import {getDiscapUserRegisterTypeUsuario, getDiscapUserRegisterTypeESPECIALISTA} from '../apis/controllers'
  ...
  export async function getInfoMedicalHistory() {
    const url = "http://localhost:8091/api/discapTrainingMedicalUser/MedicalHistory";
    try {
      const response = await axios.get(url)
      const data = response.data; // Obtenemos los datos de la respuesta de la API
      return data;
  ...
  export async function getInfoMedicalHistoryByUser() {
    const url = `http://localhost:8091/api/discapTrainingMedicalUser/MedicalHistory/discapUser?discapuserID=${sessionStorage.getItem('cedulaUser')}`;
    try {
      const response = await axios.get(url)
